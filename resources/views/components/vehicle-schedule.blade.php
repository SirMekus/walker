@props(['vehicles', 'daysInWeek'])

<table class="table">
    <thead>
        <tr>
            <th class="px-5 ps-5 border-end border-2">
                <span class='pe-5'>Vehicles</span>
            </th>
        ...
        <tr class='border-bottom-3'>
                ...
                <div class="d-flex">
                    <div class="flex-shrink-0">
                        <img class="img-thumbnail" width="100" height="100"
                            src="{{asset('storage/vehicles/'.$vehicle->image)}}" />
                    </div>
                    <div class="flex-grow-1 ms-1">
                        <span class='fw-bold fs-6'>{{$vehicle->name}} {{$vehicle->model}}</span>

                        <span class='d-block'>{{$vehicle->description}}</span>

                        <span @class(['d-block fw-bold', 'text-danger'=> !empty($vehicle->status),
                            'text-success' => empty($vehicle->status),
                            ])>
                            {{empty($vehicle->status) ? "Great Condition" : $vehicle->status}}
                        </span>
                    </div>
                </div>
            </td>
            ...
            <td class="px-5" colspan="{{$schedule->start_date->diffInDays($schedule->end_date)+1}}">
                <a href="{{route('schedule_overview', ['id'=>$schedule->id])}}"
                    ...
                    <span class='d-flex d-block fw-bold'>{{$schedule->driver->name['firstname']}}
